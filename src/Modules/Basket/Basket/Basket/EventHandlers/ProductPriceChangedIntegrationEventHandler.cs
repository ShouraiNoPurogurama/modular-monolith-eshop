    ...
    : IConsumer<ProductPriceChangedIntegrationEvent>
{
    public async Task Consume(ConsumeContext<ProductPriceChangedIntegrationEvent> context)
    {
        ...
        if (!result.IsSuccess)
        {
            logger.LogError("Error updating price in basket for product id: {ProductId}", command.ProductId);
        }
        
        logger.LogInformation("Price for product id {ProductId} updated in basket", command.ProductId);
    }
}